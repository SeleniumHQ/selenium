...
class FindAllBy(object):
    def __init__(self, by=By.ID, value=None, timeout=30):
        """
        Creates a new instance of FindAllBy. FindAllBy is used explicitly on PageObjects.
        Takes a locator value and upon __get__ returns any matching elements from the DOM
        ...
        .. code-block:: python
            ...
            class Home(PageObject):
                ...
                elements = FindAllBy(By.ID, 'qs', 20)
                more_elements = FindAllBy(By.CLASS_NAME, 'elementsList', 60)
    ...
    def __get__(self, obj, *args):
        if not isinstance(obj, PageObject):
            raise WebDriverException("FindAllBy can only be used on a PageObject")
        self.elements = WebDriverWait(obj.driver, self.timeout).until(
            lambda e: e.find_elements(by=self.by, value=self.value))
