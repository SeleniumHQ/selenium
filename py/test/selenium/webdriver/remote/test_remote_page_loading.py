...
def setup_module(module):
    ...
    try:
        ...
        RemotePageLoadingTests.server_proc = subprocess.Popen(
    ...
    RemotePageLoadingTests.driver = webdriver.Remote(desired_capabilities = webdriver.DesiredCapabilities.FIREFOX)
...
def teardown_module(module):
    try:
        RemotePageLoadingTests.driver.quit()
    except AttributeError:
        pass
    try:
        RemotePageLoadingTests.webserver.stop()
    except AttributeError:
        pass    
    # FIXME: This does not seem to work, the server process lingers
    try:
        os.kill(RemotePageLoadingTests.server_proc.pid, signal.SIGTERM)
        time.sleep(5)
    except:
        pass
