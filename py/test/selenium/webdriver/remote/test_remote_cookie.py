...
def setup_module(module):
    ...
    try:
        ...
        RemoteCookieTest.server_proc = subprocess.Popen(
...
class RemoteCookieTest(cookie_tests.CookieTest):
    pass


def teardown_module(module):
    try:
        RemoteCookieTest.driver.quit()
    except AttributeError:
        pass
    try:
        RemoteCookieTest.webserver.stop()
    except AttributeError:
        pass    
    # FIXME: This does not seem to work, the server process lingers
    try:
        os.kill(RemoteCookieTest.server_proc.pid, signal.SIGTERM)
        time.sleep(5)
    except:
        pass
