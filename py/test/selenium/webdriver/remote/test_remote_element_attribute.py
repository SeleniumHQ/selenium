...
def setup_module(module):
    ...
    try:
        ...
        RemoteElementAttributeTests.server_proc = subprocess.Popen(
    ...
    webserver.start()
    RemoteElementAttributeTests.webserver = webserver
    RemoteElementAttributeTests.driver = webdriver.Remote(desired_capabilities = webdriver.DesiredCapabilities.FIREFOX)


class RemoteElementAttributeTests(element_attribute_tests.ElementAttributeTests):
    pass


def teardown_module(module):
    try:
        RemoteElementAttributeTests.driver.quit()
    except AttributeError:
        pass
    try:
        RemoteElementAttributeTests.webserver.stop()
    except AttributeError:
        pass    
    # FIXME: This does not seem to work, the server process lingers
    try:
        os.kill(RemoteElementAttributeTests.server_proc.pid, signal.SIGTERM)
        time.sleep(5)
    except:
        pass
