...
def test_any_of_true(driver, pages):
    ...
    WebDriverWait(driver, 0.1).until(
        EC.any_of(EC.title_is("Nope"), EC.title_is("Hello WebDriver"))
    )
...
def test_any_of_false(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.1).until(
            EC.any_of(EC.title_is("Nope"), EC.title_is("Still Nope"))
        )
...
def test_all_of_false(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.1).until(
            EC.all_of(EC.title_is("Nope"), EC.title_is("Still Nope"))
        )
...
def test_none_of_true(driver, pages):
    pages.load("simpleTest.html")
    WebDriverWait(driver, 0.1).until(
        EC.none_of(EC.title_is("Nope"), EC.title_is("Still Nope"))
    )
...
def test_none_of_false(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.1).until(
            EC.none_of(EC.title_is("Nope"), EC.title_is("Hello WebDriver"))
        )
