...
class TestApiExample(object):
    ...
    def testGetTitle(self, driver, pages):
        pages.load("simpleTest.html")
        title = driver.title
        assert "Hello WebDriver" == title

    def testGetCurrentUrl(self, driver, pages, webserver):
        pages.load("simpleTest.html")
        url = driver.current_url
        assert webserver.where_is('simpleTest.html') == url

    def testFindElementsByXPath(self, driver, pages):
        pages.load("simpleTest.html")
        elem = driver.find_element_by_xpath("//h1")
        assert "Heading" == elem.text

    def testFindElementByXpathThrowNoSuchElementException(self, driver, pages):
        pages.load("simpleTest.html")
    ...
    def testFindElementByIdInElementContext(self, driver, pages):
        pages.load("nestedElements.html")
        elem = driver.find_element_by_name("form2")
        sub_elem = elem.find_element_by_id("2")
        assert "selectomatic" == sub_elem.get_attribute("name")

    def testFindElementByXpathInElementContext(self, driver, pages):
        pages.load("nestedElements.html")
        elem = driver.find_element_by_name("form2")
        sub_elem = elem.find_element_by_xpath("select")
        assert "2" == sub_elem.get_attribute("id")

    def testFindElementByXpathInElementContextNotFound(self, driver, pages):
        pages.load("nestedElements.html")
        elem = driver.find_element_by_name("form2")
        with pytest.raises(NoSuchElementException):
            elem.find_element_by_xpath("div")

    def testShouldBeAbleToEnterDataIntoFormFields(self, driver, pages):
        pages.load("xhtmlTest.html")
    ...
    def testFindElementByTagName(self, driver, pages):
        pages.load("simpleTest.html")
    ...
    def testFindElementByTagNameWithinElement(self, driver, pages):
        pages.load("simpleTest.html")
    ...
    def testSwitchFrameByName(self, driver, pages):
    ...
    def testIsEnabled(self, driver, pages):
    ...
    def testGetImplicitAttribute(self, driver, pages):
        pages.load("nestedElements.html")
        elems = driver.find_elements_by_xpath("//option")
        assert len(elems) >= 3
    ...
    def testExecuteSimpleScript(self, driver, pages):
        pages.load("xhtmlTest.html")
        title = driver.execute_script("return document.title;")
        assert "XHTML Test Page" == title

    def testExecuteScriptAndReturnElement(self, driver, pages):
        pages.load("xhtmlTest.html")
        elem = driver.execute_script("return document.getElementById('id1');")
        assert "WebElement" in str(type(elem))

    def testExecuteScriptWithArgs(self, driver, pages):
        pages.load("xhtmlTest.html")
        result = driver.execute_script("return arguments[0] == 'fish' ? 'fish' : 'not fish';", "fish")
        assert "fish" == result

    def testExecuteScriptWithMultipleArgs(self, driver, pages):
        pages.load("xhtmlTest.html")
        result = driver.execute_script(
            "return arguments[0] + arguments[1]", 1, 2)
        assert 3 == result

    def testExecuteScriptWithElementArgs(self, driver, pages):
    ...
    def testFindElementsByPartialLinkText(self, driver, pages):
        pages.load("xhtmlTest.html")
        elem = driver.find_element_by_partial_link_text("new window")
        elem.click()

    def testIsElementDisplayed(self, driver, pages):
        pages.load("javascriptPage.html")
    ...
    def testChangeWindowSize(self, driver, pages):
        pages.load("blank.html")
