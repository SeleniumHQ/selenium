...
def test_should_fail_with_invalid_selector_exception(driver, pages):
    ...
    with pytest.raises(InvalidSelectorException):
        WebDriverWait(driver, 0.7).until(
            EC.presence_of_element_located((By.XPATH, "//*[contains(@id,'something'"))
        )
...
def test_should_wait_until_at_least_one_visible_elements_is_found_when_searching_for_many(
    driver, pages
):
    ...
    elements = WebDriverWait(driver, 3).until(
        wait_for_two_elements((By.CLASS_NAME, "redbox"))
    )
...
def test_should_fail_to_find_visible_elements_when_explicit_waiting(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(
            EC.visibility_of_any_elements_located((By.CLASS_NAME, "redbox"))
        )


def test_should_wait_until_all_visible_elements_are_found_when_searching_for_many(
    driver, pages
):
    pages.load("hidden_partially.html")
    add_visible = driver.find_element(By.ID, "addVisible")
    ...
    elements = WebDriverWait(driver, 3).until(
        EC.visibility_of_all_elements_located((By.CLASS_NAME, "redbox"))
    )
    assert len(elements) == 2
...
def test_should_wait_only_as_long_as_timeout_specified_when_implicit_waits_are_set(
    driver, pages
):
    pages.load("dynamic.html")
...
def test_wait_until_not_should_not_fail_if_produce_ignored_exception(driver, pages):
    ignored = (InvalidElementStateException, StaleElementReferenceException)
    assert WebDriverWait(driver, 1, 0.7, ignored_exceptions=ignored).until_not(
        throw_sere
    )
...
def test_expected_condition_visibility_of_element_located(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(
            EC.visibility_of_element_located((By.ID, "clickToHide"))
        )
...
def test_expected_condition_text_to_be_present_in_element(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(
            EC.text_to_be_present_in_element((By.ID, "unwrappable"), "Expected")
        )
...
def test_expected_condition_text_to_be_present_in_element_value(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 1).until(
            EC.text_to_be_present_in_element_value((By.ID, "inputRequired"), "Expected")
        )
    driver.execute_script(
    ...
    assert "Example Expected text" == driver.find_element(
        By.ID, "inputRequired"
    ).get_attribute("value")
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 1).until(
            EC.text_to_be_present_in_element_attribute(
                (By.ID, "inputRequired"), "value", "Expected"
            )
        )
    driver.execute_script(
        "setTimeout(function(){document.getElementById('inputRequired').value = 'Example Expected text'}, 200)"
...
def test_expected_condition_frame_to_be_available_and_switch_to_it_by_locator(
    driver, pages
):
    pages.load("blank.html")
    with pytest.raises(TimeoutException):
...
def test_expected_condition_invisiblity_of_element_located(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(
            EC.invisibility_of_element_located((By.ID, "clickToHide"))
        )
    driver.execute_script("delayedShowHide(200, false)")
...
def test_expected_condition_element_to_be_clickable(driver, pages):
    ...
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(
            EC.element_to_be_clickable((By.ID, "clickToHide"))
        )
    ...
    element.click()
    WebDriverWait(driver, 4.5).until(
        EC.invisibility_of_element_located((By.ID, "clickToHide"))
    )
    assert element.is_displayed() is False
