...
def test_add_and_remove_virtual_authenticator(driver, pages):
    ...
    assert (
        get_assertion_for(driver, result["credential"]["rawId"]).get("status", "")
        == "OK"
    )
...
def test_get_credentials(driver, pages):
    ...
    assert (
        credential1.is_resident_credential
    ), "Credential1 should be resident credential"
    ...
    assert (
        credential2.is_resident_credential is False
    ), "Credential2 should not be resident credential"
    assert credential2.private_key is not None, "Credential2 should have private key"
    # Non-resident credentials don't save RP ID
    assert (
        credential2.rp_id is None
    ), "Credential2 should not have RP ID. Since it's not resident credential"
    assert (
