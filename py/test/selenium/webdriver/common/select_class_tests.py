...
@pytest.mark.xfail_firefox(
    reason="https://bugzilla.mozilla.org/show_bug.cgi?id=1429403"
)
...
def test_deselect_by_value_single(driver, pages):
    ...
    for select in [singleSelectValues1, singleSelectValues2]:
        with pytest.raises(NotImplementedError):
            Select(driver.find_element(By.NAME, select["name"])).deselect_by_value(
                select["values"][0].lower()
            )
...
def test_deselect_by_visible_text_single(driver, pages):
    ...
    for select in [singleSelectValues1, singleSelectValues2]:
        with pytest.raises(NotImplementedError):
            Select(
                driver.find_element(By.NAME, select["name"])
            ).deselect_by_visible_text(select["values"][0])
...
def test_get_all_selected_options_multiple(driver, pages):
    pages.load("formPage.html")
    opts = Select(
        driver.find_element(By.NAME, multiSelectValues1["name"])
    ).all_selected_options
    assert len(opts) == 2
    assert opts[0].text, multiSelectValues1["values"][0]
    assert opts[1].text, multiSelectValues1["values"][2]
    opts = Select(
        driver.find_element(By.NAME, multiSelectValues2["name"])
    ).all_selected_options
    assert len(opts) == 0
...
def test_deselect_by_text_non_existent(driver, pages):
    ...
    for select in [multiSelectValues1, multiSelectValues2]:
        with pytest.raises(NoSuchElementException):
            Select(
                driver.find_element(By.NAME, select["name"])
            ).deselect_by_visible_text("not there")
