...
class TestAPIRequestContext:
    ...
    def test_request_initialized(self, driver):
        assert driver.request is not None

    def test_request_returns_same_instance(self, driver):
        first = driver.request
        second = driver.request
        assert first is second

    def test_get_request(self, driver, pages):
        response = driver.request.get(pages.url("simpleTest.html"))
        assert response.status == 200
        assert response.ok
        assert "html" in response.text().lower()

    def test_get_nonexistent_page(self, driver, pages):
        response = driver.request.get(pages.url("nonexistent_page_xyz.html"))
        assert response.status == 404
        assert not response.ok

    def test_response_status_text(self, driver, pages):
        response = driver.request.get(pages.url("simpleTest.html"))
    ...
    def test_head_request(self, driver, pages):
        response = driver.request.head(pages.url("simpleTest.html"))
        assert response.status == 200
        assert response.body() == b""

    def test_post_json(self, driver, pages):
    ...
    def test_post_form_data(self, driver, pages):
        response = driver.request.post(
            pages.url("echo_body"),
            data={"field": "value"},
        )
    ...
    def test_post_form_kwarg(self, driver, pages):
        response = driver.request.post(
            pages.url("echo_body"),
    ...
    def test_browser_cookies_sent_with_request(self, driver, pages):
    ...
    def test_response_cookies_synced_to_browser(self, driver, pages):
    ...
    def test_response_json(self, driver, pages):
        response = driver.request.get(pages.url("echo_json"))
        data = response.json()
        assert isinstance(data, dict)
        assert data["status"] == "ok"

    def test_response_text(self, driver, pages):
        response = driver.request.get(pages.url("echo_json"))
        text = response.text()
        assert isinstance(text, str)
        assert "ok" in text

    def test_response_body_bytes(self, driver, pages):
        response = driver.request.get(pages.url("echo_json"))
        body = response.body()
        assert isinstance(body, bytes)
        assert b"ok" in body

    def test_response_dispose(self, driver, pages):
        response = driver.request.get(pages.url("echo_json"))
        assert len(response.body()) > 0
        response.dispose()
        assert response.body() == b""

    def test_response_headers(self, driver, pages):
        response = driver.request.get(pages.url("echo_json"))
        assert "content-type" in response.headers

    def test_custom_headers(self, driver, pages):
    ...
    def test_base_url(self, driver, pages):
    ...
    def test_isolated_context_no_browser_sync(self, driver, pages):
        driver.delete_all_cookies()
    ...
    def test_storage_state_export(self, driver):
    ...
    def test_storage_state_to_file(self, driver):
    ...
    def test_new_context_with_storage_state(self, driver):
    ...
    def test_fetch_with_custom_method(self, driver, pages):
        response = driver.request.fetch(pages.url("simpleTest.html"), method="GET")
        assert response.status == 200
        assert response.ok

    def test_fail_on_status_code_raises_on_404(self, driver, pages):
    ...
    def test_fail_on_status_code_no_raise_on_200(self, driver, pages):
        response = driver.request.get(
    ...
    def test_fail_on_status_code_instance_default(self, driver, pages):
    ...
    def test_fail_on_status_code_per_request_overrides_default(self, driver, pages):
        ctx = APIRequestContext(driver, fail_on_status_code=True)
        response = ctx.get(
            pages.url("nonexistent_page_xyz.html"),
