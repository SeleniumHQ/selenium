...
def test_should_always_focus_on_the_top_most_frame_after_anavigation_event(
    driver, pages
):
...
def test_should_not_automatically_switch_focus_to_an_iframe_when_apage_containing_them_is_loaded(
    driver, pages
):
...
def test_should_be_able_to_switch_to_aframe_using_apreviously_located_web_element(
    driver, pages
):
    pages.load("frameset.html")
...
def test_should_be_able_to_switch_to_an_iframe_using_apreviously_located_web_element(
    driver, pages
):
    pages.load("iframes.html")
...
def test_frame_searches_should_be_relative_to_the_currently_selected_frame(
    driver, pages
):
    pages.load("frameset.html")
...
def test_should_throw_frame_not_found_exception_looking_up_sub_frames_with_super_frame_names(
    driver, pages
):
    pages.load("frameset.html")
    driver.switch_to.frame(driver.find_element(By.NAME, "fourth"))
...
def test_should_be_able_to_switch_to_parent_frame_from_asecond_level_frame(
    driver, pages
):
    pages.load("frameset.html")
    driver.switch_to.frame(driver.find_element(By.NAME, "fourth"))
...
def test_should_continue_to_refer_to_the_same_frame_once_it_has_been_selected(
    driver, pages
):
    pages.load("frameset.html")
    ...
    WebDriverWait(driver, 3).until(
        EC.text_to_be_present_in_element((By.XPATH, "//p"), "Success!")
    )
...
@pytest.mark.xfail_safari
def test_should_focus_on_the_replacement_when_aframe_follows_alink_to_a_top_targeted_page(
    driver, pages
):
    pages.load("frameset.html")
    driver.switch_to.frame(0)
    ...
    WebDriverWait(driver, 3).until(
        EC.presence_of_element_located((By.ID, "only-exists-on-xhtmltest"))
    )


def test_should_allow_auser_to_switch_from_an_iframe_back_to_the_main_content_of_the_page(
    driver, pages
):
    pages.load("iframes.html")
    driver.switch_to.frame(0)
...
def get_text_of_greeting_element(driver):
    return (
        ...
        .until(EC.presence_of_element_located((By.ID, "greeting")))
        .text
    )
...
def test_get_current_url_returns_top_level_browsing_context_url_for_iframes(
    driver, pages
):
    pages.load("iframes.html")
...
def test_should_be_able_to_switch_to_the_top_if_the_frame_is_deleted_from_under_us(
    driver, pages
):
...
def test_should_be_able_to_switch_to_the_top_if_the_frame_is_deleted_from_under_us_with_frame_index(
    driver, pages
):
    pages.load("frame_switching_tests/deletingFrame.html")
    iframe = 0
...
def test_should_be_able_to_switch_to_the_top_if_the_frame_is_deleted_from_under_us_with_webelement(
    driver, pages
):
    pages.load("frame_switching_tests/deletingFrame.html")
...
def test_get_should_switch_to_default_context(driver, pages):
    ...
    driver.find_element(
        By.ID, "cheese"
    )  # Found on formPage.html but not on iframes.html.
    pages.load(
        "iframes.html"
    )  # This must effectively switch_to.default_content(), too.
    driver.find_element(By.ID, "iframe1")
