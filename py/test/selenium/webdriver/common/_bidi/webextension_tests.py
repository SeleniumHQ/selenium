...
"""Mirror of ``../bidi/webextension_tests.py`` using the generated ``_bidi`` commands, not the ``driver`` facade."""
...
def install_extension(driver, extension_data):
    result = WebExtension(driver).install(extension_data=extension_data)
    assert result.extension == EXTENSION_ID
...
def uninstall_extension_and_verify_extension_uninstalled(driver, extension_id):
    WebExtension(driver).uninstall(extension=extension_id)

    BrowsingContext(driver).reload(context=driver.current_window_handle)
...
class TestFirefoxWebExtension:
    ...
    def test_install_extension_path(self, driver, pages):
        ...
        extension_id = install_extension(driver, ExtensionPath(path=path))
    ...
    def test_install_archive_extension_path(self, driver, pages):
        ...
        extension_id = install_extension(driver, ExtensionArchivePath(path=path))
        verify_extension_injection(driver, pages)
        uninstall_extension_and_verify_extension_uninstalled(driver, extension_id)
    ...
    def test_install_base64_extension_path(self, driver, pages):
        ...
        extension_id = install_extension(driver, ExtensionBase64Encoded(value=base64_encoded))
...
class TestChromiumWebExtension:
    ...
    def test_install_extension_path(self, chromium_driver, pages_chromium):
        # Chromium assigns its own extension id, so we do not assert the Firefox EXTENSION_ID here.
        path = os.path.join(EXTENSIONS, EXTENSION_PATH)
        extension_id = WebExtension(chromium_driver).install(extension_data=ExtensionPath(path=path)).extension
