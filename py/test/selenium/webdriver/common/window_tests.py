...
def test_should_set_the_position_of_the_current_window(driver):
    ...
    WebDriverWait(driver, 2).until(
        lambda d: d.get_window_position()["x"] != position["x"]
        and d.get_window_position()["y"] != position["y"]
    )
...
@pytest.mark.xfail_safari(
    raises=WebDriverException, reason="Get Window Rect command not implemented"
)
@pytest.mark.xfail_chrome
...
def test_should_set_the_rect_of_the_current_window(driver):
    ...
    driver.set_window_rect(
        x=target_x, y=target_y, width=target_width, height=target_height
    )

    WebDriverWait(driver, 2).until(
        lambda d: d.get_window_position()["x"] != rect["x"]
        and d.get_window_position()["y"] != rect["y"]
    )
