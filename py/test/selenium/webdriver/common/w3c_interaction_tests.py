...
def test_pen_pointer_properties(driver, pages):
    ...
    actions.pointer_action.move_to(pointerArea).pointer_down(
        pressure=0.36, tilt_x=-72, tilt_y=9, twist=86
    ).move_to(pointerArea, x=10, y=40).pointer_up().move_to(pointerArea, x=10, y=50)
    actions.perform()
...
def test_touch_pointer_properties(driver, pages):
    ...
    touch_chain.pointer_action.move_to(pointerArea, x=50, y=25).pointer_down(
...
def _perform_drag_and_drop_with_mouse(driver, pages):
    ...
    actions = ActionBuilder(driver)
    pointer = actions.pointer_action
    pointer.click_and_hold(toDrag).move_to(
        driver.find_element(By.ID, "leftitem-4")
    ).move_to(dragInto).release()
