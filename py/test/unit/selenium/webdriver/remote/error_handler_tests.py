...
def test_does_not_raise_exception_on_success(handler):
    assert handler.check_response({"status": ErrorCode.get("SUCCESS")}) is None
    assert handler.check_response({}) is None


@pytest.mark.parametrize("code", ErrorCode.get("NO_SUCH_ELEMENT"))
...
@pytest.mark.parametrize("code", ErrorCode.get("ELEMENT_NOT_VISIBLE"))
def test_raises_exception_for_element_not_visible(handler, code):
    with pytest.raises(exceptions.ElementNotVisibleException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("INVALID_ELEMENT_STATE"))
...
@pytest.mark.parametrize("code", ErrorCode.get("UNKNOWN_ERROR"))
def test_raises_exception_for_unknown_error(handler, code):
    with pytest.raises(exceptions.WebDriverException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("ELEMENT_IS_NOT_SELECTABLE"))
...
@pytest.mark.parametrize("code", ErrorCode.get("UNABLE_TO_SET_COOKIE"))
def test_raises_exception_for_unable_to_set_cookie(handler, code):
    with pytest.raises(exceptions.UnableToSetCookieException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("UNEXPECTED_ALERT_OPEN"))
...
@pytest.mark.parametrize("code", ErrorCode.get("INVALID_XPATH_SELECTOR"))
def test_raises_exception_for_invalid_xpath_selector(handler, code):
    with pytest.raises(exceptions.InvalidSelectorException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("INVALID_XPATH_SELECTOR_RETURN_TYPER"))
def test_raises_exception_for_invalid_xpath_selector_return_typer(handler, code):
    with pytest.raises(exceptions.InvalidSelectorException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("ELEMENT_NOT_INTERACTABLE"))
...
@pytest.mark.parametrize("code", ErrorCode.get("INSECURE_CERTIFICATE"))
...
@pytest.mark.parametrize("code", ErrorCode.get("ELEMENT_CLICK_INTERCEPTED"))
...
@pytest.mark.parametrize("code", ErrorCode.get("UNKNOWN_METHOD"))
def test_raises_exception_for_unknown_method(handler, code):
    with pytest.raises(exceptions.UnknownMethodException):
        handler.check_response({"status": code, "value": "foo"})


@pytest.mark.parametrize("code", ErrorCode.get("METHOD_NOT_ALLOWED"))
...
def test_relays_exception_stacktrace(handler, key):
    ...
    value = {key: [stacktrace], "message": "very bad", "error": ErrorCode.get("UNKNOWN_METHOD")[0]}
