...
from selenium.webdriver.msedge.options import Options
...
def test_get_extensions_from_extension_files(options, mocker):
    ...
    mocker.patch(
        'selenium.webdriver.msedge.options.open'.format(__name__)).return_value = open(null)
...
def test_starts_with_default_capabilities(options):
    ...
    assert options._caps == DesiredCapabilities.MSEDGE
