 ...
 */
public class SpecifiedPathBrowserLauncherTestCase extends TestCase {

    public void testBrowserLaunchIfWindows() {
        final String[] commandString = new String[1];
        BrowserLauncher launcher = new SpecifiedPathBrowserLauncher("blort") {
            protected void exec(String command) {
                commandString[0] = command;
            }
        };
        launcher.launch("foo bar");
        assertEquals("blort foo bar", commandString[0]);
