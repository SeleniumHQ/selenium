 ...
 */
public class WindowsDefaultBrowserLauncherTestCase extends TestCase {

    public void testBrowserLaunchIfWindows() {
        final String[] commandString = new String[1];
        BrowserLauncher launcher = new WindowsDefaultBrowserLauncher() {
            protected void exec(String command) {
                commandString[0] = command;
            }
        };
        launcher.launch("foo bar");
        assertEquals("cmd /c start foo bar", commandString[0]);
