...
var getIndent = function () { return '    '.repeat(numIndents); }; // 4 spaced indents
...
var fixCamelCase = function (name) {
    ...
    return prefix + refined.replace(/HTML|XML|WML|API/i, function (match) { return match.toUpperCase(); });
};
...
var emitDescription = function (description) {
    if (description)
        getCommentLines(description).map(function (l) { return emitLine(l); });
};
    ...
    if ('$ref' in prop)
        ...
        if (!prop.properties) {
            ...
            objStr += prop.properties
                .map(function (p) { return "" + getIndent() + getPropertyDef(interfaceName, p) + ";\n"; })
                .join('');
            numIndents--;
            objStr += getIndent() + "}";
            return objStr;
...
var emitInterface = function (interfaceName, props) {
    ...
    props ? props.forEach(function (prop) { return emitProperty(interfaceName, prop); }) : emitLine('[key: string]: string;');
    emitCloseBlock();
};
...
var toTitleCase = function (str) { return str[0].toUpperCase() + str.substr(1); };
var toCmdRequestName = function (commandName) { return toTitleCase(commandName) + "Request"; };
var toCmdResponseName = function (commandName) { return toTitleCase(commandName) + "Response"; };
...
var toEventPayloadName = function (eventName) { return toTitleCase(eventName) + "Event"; };
...
var isWeakInterface = function (params) {
    return params.every(function (p) { return !!p.optional; });
};
var getCommandMapping = function (command, domainName, modulePrefix) {
    ...
    var responseType = command.returns ? prefix + toCmdResponseName(command.name) : 'void';
    return {
        type: 'object',
...
var emitMapping = function (moduleName, protocolModuleName, domains) {
    ...
    var eventDefs = flatten(domains.map(function (d) {
        var domainName = toTitleCase(d.domain);
        return (d.events || []).map(function (e) { return getEventMapping(e, domainName, protocolModulePrefix); });
    }));
    emitInterface('Events', eventDefs);
    emitLine();
    var commandDefs = flatten(domains.map(function (d) {
        var domainName = toTitleCase(d.domain);
        return (d.commands || []).map(function (c) { return getCommandMapping(c, domainName, protocolModulePrefix); });
    }));
...
var emitApiCommand = function (command, domainName, modulePrefix) {
    ...
    var params = command.parameters ? "params: " + prefix + toCmdRequestName(command.name) : '';
    var response = command.returns ? "" + prefix + toCmdResponseName(command.name) : 'void';
...
var emitApiEvent = function (event, domainName, modulePrefix) {
    var prefix = modulePrefix + "." + domainName + ".";
    emitDescription(event.description);
    var params = event.parameters ? "params: " + prefix + toEventPayloadName(event.name) : '';
...
var emitDomainApi = function (domain, modulePrefix) {
    ...
    if (domain.commands)
        domain.commands.forEach(function (c) { return emitApiCommand(c, domainName, modulePrefix); });
    if (domain.events)
        domain.events.forEach(function (e) { return emitApiEvent(e, domainName, modulePrefix); });
    emitCloseBlock();
};
var emitApi = function (moduleName, protocolModuleName, domains) {
    ...
    emitLine();
    var protocolModulePrefix = toTitleCase(protocolModuleName);
    domains.forEach(function (d) { return emitDomainApi(d, protocolModulePrefix); });
    emitCloseBlock();
    emitLine();
    emitLine("export default " + moduleName + ";");
};
