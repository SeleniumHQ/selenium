...
async function buildProfile (template, extensions) {
	...
	if (extensions.length) {
		dir = await io.tmpDir();
		if (template) {
			await io.copyDir(/** @type {string} */ (template), dir,
...
class Options extends Capabilities {
	 ...
	 * @return {!Options} A self reference
	 */
	enableMobile (androidPackage = "org.mozilla.firefox", androidActivity = null,
		deviceSerial = null) {
...
function configureExecutor (executor) {
	executor.defineCommand(ExtensionCommand.GET_CONTEXT, "GET",
		"/session/:sessionId/moz/context");

	executor.defineCommand(ExtensionCommand.SET_CONTEXT, "POST",
		"/session/:sessionId/moz/context");

	executor.defineCommand(ExtensionCommand.INSTALL_ADDON, "POST",
		"/session/:sessionId/moz/addon/install");

	executor.defineCommand(ExtensionCommand.UNINSTALL_ADDON, "POST",
		"/session/:sessionId/moz/addon/uninstall");

	executor.defineCommand(ExtensionCommand.FULL_PAGE_SCREENSHOT, "GET",
...
class Driver extends webdriver.WebDriver {
	...
	static createSession (opt_config, opt_executor) {
		let caps = opt_config instanceof Capabilities ? opt_config : new Options(
			opt_config);
	...
	setContext (ctx) {
		return this.execute(
			new command.Command(ExtensionCommand.SET_CONTEXT).setParameter("context",
				ctx));
	}
	...
	async uninstallAddon (id) {
		...
		return this.execute(
			new command.Command(ExtensionCommand.UNINSTALL_ADDON).setParameter("id",
				id));
	}
	...
	takeFullPageScreenshot () {
		return this.execute(
			new command.Command(ExtensionCommand.FULL_PAGE_SCREENSHOT));
	}
}
...
class Channel {
	...
	locate () {
		...
		switch (process.platform) {
			case "darwin":
				found =
					io.exists(this.darwin_).then(
						(exists) => (exists ? this.darwin_ : io.findInPath("firefox")));
				break;

			case "win32":
				found =
					findInProgramFiles(this.win32_).then(
 ...
 */
Channel.BETA =
	new Channel("/Applications/Firefox.app/Contents/MacOS/firefox",
 ...
 */
Channel.NIGHTLY =
	new Channel("/Applications/Firefox Nightly.app/Contents/MacOS/firefox",
		"Nightly\\firefox.exe");
