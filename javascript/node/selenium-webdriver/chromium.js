...
function configureExecutor (executor, vendorPrefix) {
	executor.defineCommand(Command.LAUNCH_APP, "POST",
		"/session/:sessionId/chromium/launch_app");
	executor.defineCommand(Command.GET_NETWORK_CONDITIONS, "GET",
		"/session/:sessionId/chromium/network_conditions");
	executor.defineCommand(Command.SET_NETWORK_CONDITIONS, "POST",
		"/session/:sessionId/chromium/network_conditions");
	executor.defineCommand(Command.DELETE_NETWORK_CONDITIONS, "DELETE",
		"/session/:sessionId/chromium/network_conditions");
	executor.defineCommand(Command.SEND_DEVTOOLS_COMMAND, "POST",
	...
	executor.defineCommand(Command.SET_PERMISSION, "POST",
		"/session/:sessionId/permissions");
	executor.defineCommand(Command.GET_CAST_SINKS, "GET",
	...
	executor.defineCommand(Command.STOP_CASTING, "POST",
...
class Extensions {
	...
	[Symbols.serialize] () {
		return this.extensions.map(function(extension) {
			...
			return io.read(/** @type {string} */ (extension)).then(
...
class Driver extends webdriver.WebDriver {
	 ...
	 */
	static createSession (caps, opt_serviceExecutor, vendorPrefix = "",
		vendorCapabilityKey = "") {
		let executor;
		let onQuit;
	...
	sendDevToolsCommand (cmd, params = {}) {
		return this.execute(
			new command.Command(Command.SEND_DEVTOOLS_COMMAND).setParameter("cmd",
				cmd).setParameter("params", params)
		);
	}
	...
	setPermission (name, state) {
		return this.execute(
			new command.Command(Command.SET_PERMISSION).setParameter("descriptor",
				{ name }).setParameter("state", state)
		);
	}
	...
	setCastSinkToUse (deviceName) {
		return this.execute(
			new command.Command(Command.SET_CAST_SINK_TO_USE).setParameter("sinkName",
				deviceName));
	}
	...
	stopCasting (deviceName) {
		return this.execute(
			new command.Command(Command.STOP_CASTING).setParameter("sinkName",
				deviceName));
	}
}
