...
module Selenium
  module WebDriver
    module Firefox
      class Options
        attr_reader :args, :prefs, :options
        attr_accessor :binary, :profile, :log_level
        ...
        #
        # @example Set the foo option to bar
        #
        #   options = Selenium::WebDriver::Firefox::Options.new
        ...
        def as_json(*)
          opts = @options
          if @profile
            opts[:profile] = @profile.is_a?(Profile) ? @profile.encoded : @profile
          end
