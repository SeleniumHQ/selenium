...
function test_xpathToUpperCase()
{
    var xpath = "//h1[@id='title']/div[p[@class='round']]/parent::a/img/text()";
    assertEquals("//H1[@id='title']/DIV[P[@class='round']]/parent::A/IMG/text()", xpathToUpperCase(xpath));
    xpath = "//div/ul[@id='albumlist']/li/@index";
    assertEquals("//DIV/UL[@id='albumlist']/LI/@index", xpathToUpperCase(xpath));

    // test forward slash and single-character element at the end of the expression
    xpath = "//a[text()='Non/sense.']";
    assertEquals("//A[text()='Non/sense.']", xpathToUpperCase(xpath));
    xpath = "//a/b";
    assertEquals("//A/B", xpathToUpperCase(xpath));
    
    xpath = "//tr[td[span[em[@class='foo']]]]/td[2]";
    assertEquals("//TR[TD[SPAN[EM[@class='foo']]]]/TD[2]", xpathToUpperCase(xpath));
    
    // borrow some testcases from the Selenium XPath locator suite. Note that
    // for the first test immediately following, the parentheses are being
    // stripped from the output. When and if I figure out how to keep them in
    // (if the parsing engine makes it possible), I'll update the function and
    // assertion.
    xpath = "(//table[@class='stylee'])//th[text()='theHeaderText']/../td";
    assertEquals("//TABLE[@class='stylee']//TH[text()='theHeaderText']/../TD"
        , xpathToUpperCase(xpath));
    xpath = "//input[@name='name2' and @value='yes']";
    assertEquals("//INPUT[@name='name2' and @value='yes']"
        , xpathToUpperCase(xpath));
    xpath = "id('content')/div";
    assertEquals("id('content')/DIV"
        , xpathToUpperCase(xpath));
    xpath = "id('anotherNested')//div[contains(@id, 'useful')]//input";
    assertEquals("id('anotherNested')//DIV[contains(@id, 'useful')]//INPUT"
        , xpathToUpperCase(xpath));
    xpath = "//a[@href=\"javascript:doFoo('a', 'b')\"]";
    assertEquals("//A[@href=\"javascript:doFoo('a', 'b')\"]"
        , xpathToUpperCase(xpath));
    xpath = "//a/b[@class='c'] | //d[@id='e']/f";
    assertEquals("//A/B[@class='c'] | //D[@id='e']/F"
        , xpathToUpperCase(xpath));
    xpath = "//div[@id='body-content']/descendant::div[child::div/span]";
    assertEquals("//DIV[@id='body-content']/descendant::DIV[DIV/SPAN]"
        , xpathToUpperCase(xpath));
    xpath = "id('foo')/descendant::td[@class='bar' and descendant-or-self::*[text()='baz']]/following-sibling::td[@class='ooka']";
    assertEquals("id('foo')/descendant::TD[@class='bar' and //*[text()='baz']]/following-sibling::TD[@class='ooka']"
        , xpathToUpperCase(xpath));
    xpath = "id('foo')/descendant::tr[descendant::p[normalize-space()='bar'] and position()=last()]/descendant::a";
    assertEquals("id('foo')/descendant::TR[descendant::P[normalize-space()='bar'] and position()=last()]/descendant::A"
        , xpathToUpperCase(xpath));
    xpath = "//div[@class='foo'][1]/descendant::*[starts-with(@id, 'bar(') and substring(@id, string-length(@id) - 2)='baz'][1]";
    assertEquals("//DIV[@class='foo'][1]/descendant::*[starts-with(@id, 'bar(') and substring(@id, string-length(@id) - 2)='baz'][1]"
        , xpathToUpperCase(xpath));
        
    // round trip an xpath in our canonical form
    xpath = "id('foo')/descendant::tr[descendant::p[normalize-space()='bar'] and position()=last()]/descendant::a";
    var xpathObj = xpathParse(xpath);
    assertEquals(xpath, xpathObj.toString());
    ...
    try {
        ...
        assertTrue(/^XPath parse/.test(e));
    }
}



function test_xpathSetNamespace()
{
    var xpath = "//h1[@id='title']/div[p[@class='round']]/parent::a/img/text()";
    assertEquals("//ns:h1[@id='title']/ns:div[ns:p[@class='round']]/parent::ns:a/ns:img/text()"
        , xpathSetNamespace(xpath, 'ns'));
    xpath = '/foo:i/bar:already/baz:have/it';
    assertEquals('/ns:i/ns:already/ns:have/ns:it'
        , xpathSetNamespace(xpath, 'ns'));
}
