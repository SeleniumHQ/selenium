...
`,Vt=Pt,Lt=i=>{let d=Object.entries(i).map(p=>({label:p[0],value:p[1]})).sort((p,x)=>x.value-p.value);return st().value(p=>p.value)(d)},Ft=(i,d,F,p)=>{var x,E;K.debug(`rendering pie chart
`+i);let $=p.db,D=Q(),f=lt($.getConfig(),D.pie),v=450,w=((E=(x=document.getElementById(d))==null?void 0:x.parentElement)==null?void 0:E.offsetWidth)??f.useWidth,C=ct(d);C.attr("viewBox",`0 0 ${w} ${v}`),ot(C,v,w,f.useMaxWidth);let z=40,_=18,O=4,m=C.append("g");m.attr("transform","translate("+w/2+","+v/2+")");let{themeVariables:c}=D,[N]=at(c.pieOuterStrokeWidth);N??(N=2);let P=f.textPosition,y=Math.min(w,v)/2-z,t=J().innerRadius(0).outerRadius(y),e=J().innerRadius(y*P).outerRadius(y*P);m.append("circle").attr("cx",0).attr("cy",0).attr("r",y+N/2).attr("class","pieOuterCircle");let s=$.getSections(),r=Lt(s),a=[c.pie1,c.pie2,c.pie3,c.pie4,c.pie5,c.pie6,c.pie7,c.pie8,c.pie9,c.pie10,c.pie11,c.pie12],n=it(a);m.selectAll("mySlices").data(r).enter().append("path").attr("d",t).attr("fill",o=>n(o.data.label)).attr("class","pieCircle");let S=0;Object.keys(s).forEach(o=>{S+=s[o]}),m.selectAll("mySlices").data(r).enter().append("text").text(o=>(o.data.value/S*100).toFixed(0)+"%").attr("transform",o=>"translate("+e.centroid(o)+")").style("text-anchor","middle").attr("class","slice"),m.append("text").text($.getDiagramTitle()).attr("x",0).attr("y",-(v-50)/2).attr("class","pieTitleText");let h=m.selectAll(".legend").data(n.domain()).enter().append("g").attr("class","legend").attr("transform",(o,A)=>{let V=_+O,R=V*n.domain().length/2,B=12*_,l=A*V-R;return"translate("+B+","+l+")"});h.append("rect").attr("width",_).attr("height",_).style("fill",n).style("stroke",n),h.data(r).append("text").attr("x",_+O).attr("y",_-O).text(o=>{let{label:A,value:V}=o.data;return $.getShowData()?`${A} [${V}]`:A})},Nt={draw:Ft},Bt={parser:St,db:Ot,renderer:Nt,styles:Vt};export{Bt as diagram};
//# sourceMappingURL=pieDiagram-47467502-FQNQ6D3F.min.js.map
