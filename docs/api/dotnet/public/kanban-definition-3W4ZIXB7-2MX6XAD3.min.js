...
`+e);let _=r.db.getData(),y=M();y.htmlLabels=!1;let l=pe(u),D=l.append("g");D.attr("class","sections");let I=l.append("g");I.attr("class","items");let g=_.nodes.filter(k=>k.isGroup),w=0,m=10,U=[],N=25;for(let k of g){let A=y?.kanban?.sectionWidth||200;w=w+1,k.x=A*w+(w-1)*m/2,k.width=A,k.y=0,k.height=A*3,k.rx=5,k.ry=5,k.cssClasses=k.cssClasses+" section-"+w;let L=await ye(D,k);N=Math.max(N,L?.labelBBox?.height),U.push(L)}let j=0;for(let k of g){let A=U[j];j=j+1;let L=y?.kanban?.sectionWidth||200,H=-L*3/2+N,T=H,J=_.nodes.filter(i=>i.parentId===k.id);for(let i of J){if(i.isGroup)throw new Error("Groups within groups are not allowed in Kanban diagrams");i.x=k.x,i.width=L-1.5*m;let s=(await ke(I,i,{config:y})).node().getBBox();i.y=T+s.height/2,await me(i),T=i.y+s.height/2+m/2}let F=A.cluster.select("rect"),O=Math.max(T-H+3*m,50)+(N-25);F.attr("height",O)}de(void 0,l,y.mindmap?.padding??W.kanban.padding,y.mindmap?.useMaxWidth??W.kanban.useMaxWidth)},"draw"),Ge={draw:Fe},Me=o(e=>{let u="";for(let r=0;r<e.THEME_COLOR_LIMIT;r++)e["lineColor"+r]=e["lineColor"+r]||e["cScaleInv"+r],ge(e["lineColor"+r])?e["lineColor"+r]=te(e["lineColor"+r],20):e["lineColor"+r]=ne(e["lineColor"+r],20);let p=o((r,d)=>e.darkMode?ne(r,d):te(r,d),"adjuster");for(let r=0;r<e.THEME_COLOR_LIMIT;r++){let d=""+(17-3*r);u+=`
  ...
  .kanban-label {
    ...
    ${Ee()}
`,"getStyles"),He=Ue,nt={db:je,renderer:Ge,parser:De,styles:He};export{nt as diagram};
//# sourceMappingURL=kanban-definition-3W4ZIXB7-2MX6XAD3.min.js.map
