...
public class TestUIElementLocators extends InternalSelenseTestBase {
  ...
  void testUIElementLocators() {
    ...
    verifyEquals(selenium.getText("ui=pageset1::linksWithId(index=1)"),
                 "this is the first element");
    verifyTrue(selenium.getText("ui=pageset1::linksWithId(index=2)").matches(
      "^this is the[\\s\\S]*second[\\s\\S]*element$"));
    verifyEquals(selenium.getText("ui=pageset1::linksWithId(index=3)"),
    ...
    verifyEquals(
      selenium
        .getEval(
          "map.getUISpecifierString(selenium.browserbot.findElement('id=firstChild'), selenium.browserbot.getDocument())"),
