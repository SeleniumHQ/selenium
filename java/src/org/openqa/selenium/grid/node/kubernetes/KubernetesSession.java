...
public class KubernetesSession extends DefaultActiveSession {
  ...
  public void stop() {
    ...
    // Delete the Job so K8s sends SIGTERM to containers (including the video sidecar). The recorder
    // writes each video straight to its final /videos/<sessionId>/ location, so there is nothing to
    // wait for or relocate here.
    deleteJob();
    super.stop();
  }
