...
public class DockerFlags implements HasRoles {
  ...
  @ConfigValue(
      ...
      name = "url",
      example = "\"" + DockerOptions.DEFAULT_DOCKER_URL + "\"")
  ...
  @Parameter(
      ...
      description =
          "Exposes devices to a container. Each device mapping declaration must have  at least the"
              + " path of the device in both host and container separated by a colon like in this"
              + " example: /device/path/in/host:/device/path/in/container",
      arity = 1,
      variableArity = true,
      splitter = NonSplittingSplitter.class)
  @ConfigValue(
  ...
  @ConfigValue(
      section = DockerOptions.DOCKER_SECTION,
      name = "video-image",
      example = DockerOptions.DEFAULT_VIDEO_IMAGE)
