...
public class InternetExplorerDriver extends RemoteWebDriver implements TakesScreenshot {
   ...
   */
  public final static String LAUNCH_API_CAPABILITY = "driverIELaunchApi";
    ...
    try {
      ...
      if (caps != null) {
        ...
        if (caps.getCapability(EXTRACT_PATH_CAPABILITY) != null) {
          ...
          if (value != null) {
            File extractPathFile = new File(value);
            builder.withExtractPath(extractPathFile);
          }
        }

        if (caps.getCapability(SILENT_CAPABILITY) != null) {
          String value = (String) caps.getCapability(SILENT_CAPABILITY);
          if (value != null) {
            Boolean silent = Boolean.valueOf(value);
            builder.withSilent(silent);
          }
        }

        if (caps.getCapability(LAUNCH_API_CAPABILITY) != null) {
          String value = (String) caps.getCapability(LAUNCH_API_CAPABILITY);
          if (value != null) {
            InternetExplorerDriverLaunchApi launchApi = InternetExplorerDriverLaunchApi.valueOf(value);
            builder.withLaunchApi(launchApi);
          }
        }
