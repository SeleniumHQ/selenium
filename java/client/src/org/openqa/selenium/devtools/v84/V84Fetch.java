...
import org.openqa.selenium.devtools.v84.fetch.model.AuthChallengeResponse;
import org.openqa.selenium.devtools.v84.fetch.model.AuthRequired;
...
public class V84Fetch implements org.openqa.selenium.devtools.idealized.fetch.Fetch {

  @Override
  public Command<Void> enable(Optional<List<RequestPattern>> patterns, boolean handleAuthRequests) {
    List<org.openqa.selenium.devtools.v84.fetch.model.RequestPattern> mapped = patterns.orElseGet(ArrayList::new).stream()
      .map(pattern -> {
        Optional<ResourceType> type = pattern.getType().map(t -> ResourceType.fromString(t.toString()));
        return new org.openqa.selenium.devtools.v84.fetch.model.RequestPattern(
  ...
  public Command<Void> fulfillRequest(RequestId requestId, HttpResponse response) {
    ...
    return Fetch.fulfillRequest(
      new org.openqa.selenium.devtools.v84.fetch.model.RequestId(requestId.toString()),
      response.getStatus(),
      Optional.of(headers),
      Optional.empty(),
