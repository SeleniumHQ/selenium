...
public class V85Fetch implements org.openqa.selenium.devtools.idealized.fetch.Fetch {

  @Override
  public Command<Void> enable(Optional<List<RequestPattern>> patterns, boolean handleAuthRequests) {
    List<org.openqa.selenium.devtools.v85.fetch.model.RequestPattern> mapped = patterns.orElseGet(ArrayList::new).stream()
      .map(pattern -> {
        Optional<ResourceType> type = pattern.getType().map(t -> ResourceType.fromString(t.toString()));
        return new org.openqa.selenium.devtools.v85.fetch.model.RequestPattern(
