...
public class NetworkInterceptor implements Closeable {
  ...
  private void handleRequest(RequestPaused incoming) {
    ...
    if (incoming.getResponseStatusCode().isPresent()
        || incoming.getResponseErrorReason().isPresent()) {
      return;
    }
    ...
    try {
      ...
      LOG.log(
          Level.WARNING,
          String.format(
              "Caught exception while handling %s: %s",
              incoming.getRequest().getUrl(), e.getMessage()),
          e);
      continueRequest(incoming);
    }
  }
