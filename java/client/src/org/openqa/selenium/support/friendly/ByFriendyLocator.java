...
public class ByFriendyLocator {
  ...
  static {
    try {
      String location =
          String.format(
              "/%s/%s",
              ByFriendyLocator.class.getPackage().getName().replace(".", "/"), "findElements.js");
  ...
  public static class FriendlyBy extends By {
      ...
      return new FriendlyBy(
          ...
          amend(
              ImmutableMap.of(
                  ...
                  "args",
                  ImmutableList.of(
                      asAtomLocatorParameter(locator), "distance", atMostDistanceInPixels))));
    }
    ...
    private JavascriptExecutor extractJsExecutor(SearchContext context) {
      ...
      throw new IllegalArgumentException(
          "Cannot find elements, since the context cannot execute JS: " + context);
    }
