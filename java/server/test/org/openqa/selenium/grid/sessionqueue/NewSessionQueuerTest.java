...
public class NewSessionQueuerTest {
  ...
  public void setUp() {
    ...
    NewSessionQueue sessionQueue = new LocalNewSessionQueue(tracer, bus, 1);
  ...
  public void shouldBeAbleToAddToQueueAndGetValidResponse() {
    ...
    bus.addListener(NEW_SESSION_REQUEST, event -> {
      ...
      try {
        ...
        bus.fire(
            new NewSessionRejectedEvent(new NewSessionErrorResponse("Error", UUID.randomUUID())));
      }
    });
    ...
    bus.addListener(NEW_SESSION_REQUEST, event -> {
      ...
      bus.fire(
          new NewSessionRejectedEvent(
              new NewSessionErrorResponse("Error", reqId)));
  ...
  public void shouldBeAbleToAddToQueueRemotely() {
    ...
    bus.addListener(NEW_SESSION_REQUEST, event -> {
      ...
      bus.fire(
          new NewSessionRejectedEvent(
              new NewSessionErrorResponse("Could not poll the queue", reqId)));
  ...
  public void shouldBeAbleToAddAgainToQueue() {
    boolean added = local.retryAddToQueue(request, UUID.randomUUID());
    assertTrue(added);
  }
  ...
  public void shouldBeAbleToAddAgainToQueueRemotely() {
    ...
    boolean added = remote.retryAddToQueue(request, UUID.randomUUID());
