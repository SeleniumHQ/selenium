...
public class WebDriverBackedSeleniumHandler implements Routable {
  ...
  private HttpResponse startNewSession(String browserString, String baseUrl, String options) {
    ...
    if (options.startsWith("webdriver.remote.sessionid")) {
      ...
      if (!"webdriver.remote.sessionid".equals(split.get(0))) {
        ...
        return sendError(
            HTTP_OK, "Unable to find existing webdriver session. Wrong parameter name: " + options);
      }
      if (split.size() != 2) {
        LOG.warning("Attempted to find webdriver id, but none specified. Bailing");
        return sendError(HTTP_OK, "Unable to find existing webdriver session. No ID specified");
      }
  ...
  private String getValue(
      String key, Optional<Map<String, List<String>>> params, HttpRequest request) {
    return params
        .map(
            data -> {
