...
import static org.openqa.selenium.remote.RemoteTags.*;
...
public class RedisDistributor extends Distributor implements Closeable {
  ...
  private static final String HOST_SET_NAME = "hosts";
  ...
  public CreateSessionResponse newSession(HttpRequest request)
    ...
    try (
      ...
      try {
        ...
        Optional<Host> selectedHost = hostSelector.selectHost(firstRequest.getCapabilities(), connection.getHosts(HOST_SET_NAME));
  ...
  public Distributor add(Node node) {
    ...
    try (JsonOutput out = JSON.newOutput(sb)) {
      ...
      connection.addHost(host, HOST_SET_NAME);
  ...
  public void remove(UUID nodeId) {
    ...
    writeLock.lock();
    try {
      connection.removeHost(nodeId, HOST_SET_NAME);
  ...
  public DistributorStatus getStatus() {
    ...
    readLock.lock();
    try {
      ImmutableSet<DistributorStatus.NodeSummary> summaries = connection.getHosts(HOST_SET_NAME).stream()
        .map(Host::asSummary)
  ...
  private void refresh(NodeStatus status) {
    ...
    Lock writeLock = lock.writeLock();
    writeLock.lock();
    Set<Host> hosts = connection.getHosts(HOST_SET_NAME);
    try {
  ...
  public void refresh() {
    ...
    writeLock.lock();
    try {
      connection.getHosts(HOST_SET_NAME).forEach(Host::runHealthCheck);
    } finally {
      writeLock.unlock();
    }
  }
  ...
  @VisibleForTesting
  UUID getNodeUri(UUID id) {
    for (Host host : connection.getHosts(HOST_SET_NAME)) {
