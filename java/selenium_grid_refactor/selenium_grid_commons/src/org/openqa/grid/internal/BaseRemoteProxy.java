...
public class BaseRemoteProxy implements RemoteProxy {
  ...
  private static final Logger log = Logger.getLogger(BaseRemoteProxy.class
      .getName());
   ...
   * <p/>
   * If maxSession is not specified, default to 1 = max number of tests running
   * at a given time will be 1.
   * <p/>
   * For each capability, maxInstances is defaulted to 1 if not specified = max
   * number of test of each capability running at a time will be 1. maxInstances
   * for firefox can be > 1. IE won't support it.
   *
   * @param request
   *          The request
   * @param registry
  ...
  public BaseRemoteProxy(RegistrationRequest request,
    ...
    this.config = mergeConfig(registry.getConfiguration().getAllParams(),
    ...
    if (url != null) {
      try {
        ...
        throw new GridException("Not a correct url to register a remote : "
            + url);
      }
    }
  ...
  private SeleniumProtocol getProtocol(DesiredCapabilities capability) {
    ...
    if (type == null) {
      ...
      try {
        ...
        throw new GridException(
            type
                + " isn't a valid protocol type for grid. See SeleniumProtocol enum.",
            e);
      }
    }
    return protocol;
  ...
  /**
   * merge the param from config 1 and 2. If a param is present in both, config2
   * value is used.
   *
   * @param configuration1
   *          The first configuration to merge (recessive)
   * @param configuration2
   *          The second configuration to merge (dominant)
  ...
  public String getId() {
    if (id == null) {
      throw new RuntimeException(
          "Bug. Trying to use the id on a proxy but it hasn't been set.");
    }
    ...
    void cleanUpAllSlots() {
      for (TestSlot slot : testSlots) {
        try {
          ...
          log.warning("Error executing the timeout when cleaning up slot "
              + slot + t.getMessage());
        }
      }
    }
    ...
    private void cleanUpSlot(TestSlot slot) {
      ...
      if (session != null) {
        ...
        if (session.isOrphaned()) {
          log.logp(Level.WARNING, "SessionCleanup", null, "session " + session
              + " has been ORPHANED and will be released");
          try {
            ((TimeoutListener) proxy).beforeRelease(session);
          } catch (IllegalStateException ignore) {
            log.log(Level.WARNING, ignore.getMessage());
  ...
  public static <T extends RemoteProxy> T getNewInstance(
      RegistrationRequest request, HubRegistryInterface registry) {
    try {
      ...
      if (proxy instanceof RemoteProxy) {
        ...
        throw new InvalidParameterException("Error: " + proxy.getClass()
   ...
   *           If the node if down or doesn't recognize the /wd/hub/status
   *           request.
   */
  public JsonObject getStatus() throws GridException {
    String url = getRemoteHost().toExternalForm() + "/wd/hub/status";
    BasicHttpRequest r = new BasicHttpRequest("GET", url);
    HttpClient client = getHttpClientFactory().getGridHttpClient(
        statusCheckTimeout, statusCheckTimeout);
    HttpHost host = new HttpHost(getRemoteHost().getHost(), getRemoteHost()
        .getPort());
    ...
    try {
      ...
      if (code == 200) {
        ...
        try {
          ...
          // ignored due it's not required from node to return anything. Just
          // 200 code is enough.
        }
