 ...
 * 
 * use the API by sending a GET to grid/api/hub/ with the content of the request
 * in JSON,specifying the parameters you're interesting in, for instance, to get
  ...
  private JsonObject getResponse(HttpServletRequest request) throws IOException {
    ...
    try {
      if (request.getInputStream() != null) {
        ...
        JsonArray keys = requestJSON != null ? requestJSON.get("configuration")
            .getAsJsonArray() : null;

        Set<String> paramsToReturn;
        HubRegistryInterface registry = getRegistry();
        Map<String, Object> allParams = registry.getConfiguration()
            .getAllParams();
        ...
        if (paramsToReturn.contains("newSessionRequestCount")) {
          res.addProperty("newSessionRequestCount",
              registry.getNewSessionRequestCount());
