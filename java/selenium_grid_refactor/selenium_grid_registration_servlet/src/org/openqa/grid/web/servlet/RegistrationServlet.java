...
public class RegistrationServlet extends RegistryBasedServlet {
  ...
  protected void process(HttpServletRequest request,
    ...
    rd.close();
    log.fine("getting the following registration request  : "
        + registrationRequest.toString());

    // getting the settings from registration
    RegistrationRequest server = RegistrationRequest
        .getNewInstance(registrationRequest.toString());
    ...
    // TODO freynaud : load template desiredCapability from the hub. Is that
    // useful?
    final RemoteProxy proxy = BaseRemoteProxy.getNewInstance(server,
        getRegistry());
  ...
  protected void reply(HttpServletResponse response, String content)
      throws IOException {
