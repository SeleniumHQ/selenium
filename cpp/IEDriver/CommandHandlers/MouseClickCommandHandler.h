...
protected:
	void MouseClickCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const LocatorMap& locator_parameters, const ParametersMap& command_parameters, Response * response) {
		...
		if (button_parameter_iterator == command_parameters.end()) {
			...
			clickAt(browser_window_handle, executor.last_known_mouse_x(), executor.last_known_mouse_y(), button);
