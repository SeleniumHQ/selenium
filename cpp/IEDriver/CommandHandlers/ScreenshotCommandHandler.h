...
private:
	...
	HRESULT ScreenshotCommandHandler::CaptureBrowser(BrowserHandle browser) {
		...
		int image_height(0);
		int image_width(0);
		HRESULT hr = this->GetDocumentDimensions(document, &image_width, &image_height);
		if (FAILED(hr)) {
			return hr;
		}
		...
		int chrome_width(0);
		int chrome_height(0);
		this->GetBrowserChromeDimensions(ie_window_handle, content_window_handle, &chrome_width, &chrome_height);
	...
	HRESULT ScreenshotCommandHandler::GetBase64Data(std::string& data) {
		...
		if (!::Base64Encode(global_lock, (int)statstg.cbSize.QuadPart, data_array, &length, ATL_BASE64_FLAG_NOCRLF)) {
			delete[] data_array;
			::GlobalUnlock(global_memory_handle);
	...
	void ScreenshotCommandHandler::GetBrowserChromeDimensions(HWND top_level_window_handle, HWND content_window_handle, int* width, int* height) {
		int top_level_window_width = 0;
		int top_level_window_height = 0;
		this->GetWindowDimensions(top_level_window_handle, &top_level_window_width, &top_level_window_height);

		int content_window_width = 0;
		int content_window_height = 0;
		this->GetWindowDimensions(content_window_handle, &content_window_width, &content_window_height);
	...
	HRESULT ScreenshotCommandHandler::GetDocumentDimensions(IHTMLDocument2* document, int* width, int* height) {
