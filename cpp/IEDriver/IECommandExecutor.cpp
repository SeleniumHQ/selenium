...
LRESULT IECommandExecutor::OnInit(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
LRESULT IECommandExecutor::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	this->DestroyWindow();
	return 0;
}
...
LRESULT IECommandExecutor::OnWait(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	if (status_code == SUCCESS && !browser->is_closing()) {
		...
		if (this->is_waiting_) {
			...
			HANDLE thread_handle = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &IECommandExecutor::WaitThreadProc, (void *)this->m_hWnd, 0, &thread_id));
...
void IECommandExecutor::AddManagedElement(IHTMLElement* element, ElementHandle* element_wrapper) {
