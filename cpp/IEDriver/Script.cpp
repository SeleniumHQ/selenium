...
void Script::AddArgument(VARIANT argument) {
	HRESULT hr = ::SafeArrayPutElement(this->argument_array_, &this->current_arg_index_, &argument);
...
int Script::GetPropertyNameList(std::wstring* property_names) {
	...
	Script get_names_script_wrapper(this->script_engine_host_, get_names_script, 1);
...
int Script::GetArrayLength(long* length) {
	...
	Script get_length_script_wrapper(this->script_engine_host_, get_length_script, 1);
