...
namespace OpenQA.Selenium.Tests.BiDi.Log;
...
internal class LogTests : BiDiTestFixture
{
    ...
    public async Task CanListenToConsoleLog()
    {
        ...
        await using var subscription = await context.Log.EntryAdded.OnAsync(tcs.SetResult);
    ...
    public async Task CanRetrieveStacktrace()
    {
        ...
        await using var subscription = await bidi.Log.EntryAdded.OnAsync(tcs.SetResult);
