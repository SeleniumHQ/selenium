...
/**
 * Provides the capability to generate various types of XPath expressions,
 * given a node in a document. Some functionality is borrowed from Selenium IDE
 * LocatorBuilders, and may eventually want to be shared code.
 *
 * @param newDocument  the document that will be analyzed to build the XPath.
 *                     The element for which the XPath is being built must
 *                     belong to this document; however, it does NOT have to be
 *                     a top level document, as would be retrievable via the
 *                     "ownerDocument" property of the element.
 */
function XPathBuilder(newDocument) {
// private
    var doc = newDocument;
    ...
    function id(id) {
        return 'id(' + id.quoteForXPath() + ')';
    }
    
    /**
     * Returns an XPath id expression.
     */
    function buildIdXPath(e) {
        return e.id ? id(e.id) : null;
    }
    
    /**
     * Returns an XPath expression that indexes a tag name across the entire
     * document.
     */
    function buildTagNameXPath(e) {
        ...
        for (var i = 0, n = elements.length; i < n; ++i) {
            if (elements[i] === e) {
                ...
                return '/descendant::' + e.tagName + '[' + (i + 1) + ']';
            }
        }
    ...
    /**
     * Returns an XPath expression "rooted" at an element with an id.
     */
    function buildIdRelativeXPath(e) {
        var path = "";
        var current = e;
        ...
        while (current != null) {
            ...
            if (current.parentNode.nodeType == 1 && // ELEMENT_NODE
                current.parentNode.getAttribute("id")) {
                return id(current.parentNode.id) + path;
            }
...
// public
    ...
    /**
     * Builds an XPath for the given element. Returns null if this builder is
     * incapable of generating an appropriate XPath.
     */
    this.build = function(e) {
        return (
            buildIdXPath(e) ||
            buildTagNameXPath(e) ||
            buildIdRelativeXPath(e));
    };
}
...
// private
    ...
    function copyBody(bodyHtml, doc) {
        ...
        // HBC - using html() seems riskier, and doesn't actually end up being
        //       any faster!
        /*
        // override jQuery's evalScript() in this scope to prevent script
        // evaluation
        function evalScript(i, elem) {
            elem.type = 'text';
        }
